...
- `src/routes/`: Route definitions (`auth.routes.ts`, `health.routes.ts`, `profile.routes.ts`, `users.routes.ts`, `weather.routes.ts`, `audit.routes.ts`, `settings.routes.ts`, `maps.routes.ts`, `contentBlocks.routes.ts`)
- `src/controllers/`: Decoupled route handlers (`auth.controller.ts` for local auth, `oauth.controller.ts` for Google OAuth, `otp.controller.ts` for OTP verification, `profile.controller.ts` for self-service credential changes, `users.controller.ts` for user management, `weather.controller.ts` — forecast lookup, `audit.controller.ts` — audit log list/summary/acknowledge, `settings.controller.ts` — public portal settings and audit retention config, `maps.controller.ts` — spatial points and RT leader lookup, `contentBlocks.controller.ts` — CMS prose/content block queries and edits)
- `src/services/`: Business logic sitting between controllers and external/data sources (`users.service.ts` — user management & role administration, `profile.service.ts` — self-service credential updates, `weather.service.ts` — caches and transforms BMKG forecasts, `audit.service.ts` — audit log writes/reads and the `AUDIT_ACTIONS` severity table, `settings.service.ts` — public portal and audit retention settings, `maps.service.ts` — spatial points querying and RT leader coordinate resolution, `contentBlocks.service.ts` — CMS prose/content block queries and edits)
- `src/middlewares/`: Express middleware (`auth.middleware.ts` for JWT, `csrf.middleware.ts` for CSRF protection, `role.middleware.ts` for role-based authorization, `turnstile.middleware.ts` for anti-bot, `rateLimit.middleware.ts` — per-route rate limiters)
- `src/utils/`: `jwt.ts` (token signing), `oauth.ts` (Google OAuth PKCE & token verification), `otp.ts` (OTP hashing & expiry), `mailer.ts` (transactional email), `turnstile.ts` (Turnstile API client), `prisma.ts` (Prisma instance — wraps `audit_logs` writes in an append-only guard, see Audit Trail below), `bmkg.ts` (BMKG API fetch + response validation), `auditLogGuard.ts` (the append-only guard's pure logic), `actor.ts` / `requestContext.ts` (pull `{id, email, role}` and `{ipAddress, userAgent}` off a request for audit entries), `lockTransactionCache.ts` (versioned TTL cache and row-level lock transaction scaffold), `comparator.ts` (canonicalized JSON deep equality for diffs and mutations), `keyedCache.ts` (generic bounded LRU keyed cache with recency tracking)
- `prisma/schema.prisma`: ORM schema (MySQL)
- `prisma/seed.ts`: Seeds default roles (`user`, `admin`, `editor`), default audit retention settings (keep-forever), default public settings (`publicSeedMap`), default content blocks (`DEFAULT_CONTENT_BLOCKS`), sample RT leaders, and spatial points
