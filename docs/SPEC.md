...
- **Admin User Management & Soft Deletion**: Only Admins can manage users and roles. Account deletion is performed via soft deletion (`deletedAt`). Soft-deleting an account immediately revokes all active refresh tokens for that user. Admins cannot delete their own account or delete another Admin account. Public registration using a soft-deleted email returns a distinct notification (`409 Conflict`) instructing the user to contact an Administrator.
- **Account Reactivation & Forced Password Change**: Admins can reactivate soft-deleted accounts by creating/adding a user with that email. Accounts created or reactivated by an Admin are trusted (`email_verified = true`) and flagged with `requires_password_change = true`. Upon first login, users receive a restricted setup token and must complete the Cloudflare Turnstile-protected password setup flow (`/setup-password`) before gaining access to standard protected routes.
