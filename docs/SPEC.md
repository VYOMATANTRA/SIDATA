...
names). 4 of these tables (`spatial_points`, `spatial_point_rt`, `rt_leaders`, `content_blocks`) are implemented
in `backend/prisma/schema.prisma` (migrations `20260819000000_add_spatial_and_rt_tables` and `20260917103839_add_content_blocks`);
the remaining 6 tables (`pages`, `chapters`, `sections`, `indicators`, `indicator_tables`, `indicator_table_rows`)
remain pending implementation. Translate into Prisma models with `@@map` when implementing.
...
| `content_blocks`                            | Prose/narrative content. `section_id` nullable (populated for normal Cerita/stat-page prose tied to the hierarchy; null for standalone blocks like hero, Sambutan Lurah, landing highlights). `block_type` ENUM (`hero`, `sambutan_lurah`, `highlight`, `prose`), `slug` (stable lookup key), `body` TEXT (tier-2 free text). `sort_order INT NULL` with `CHECK (section_id IS NOT NULL OR sort_order IS NULL)` — ordering only means something within a section; standalone blocks are unique named slots with no siblings to sort against. Read via public `GET /api/content-blocks` and `GET /api/content-blocks/:slug` (in-memory cached); mutated via `PATCH /api/content-blocks/:slug` (Editor or Admin only, CSRF-protected, logged in `audit_logs` as `content_block.updated`). |
