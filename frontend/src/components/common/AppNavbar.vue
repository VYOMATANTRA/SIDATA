...
const containerClasses = computed(() => {
  ...
  return [
    ...
    props.sticky ? 'sticky top-0 z-40 shadow-xs' : 'relative',
  ];
});

const subtitleClasses = computed(() => {
  return isNavy.value ? 'text-slate-200/90' : 'text-slate-400';
});

const titleClasses = computed(() => {
  return isNavy.value ? 'text-white' : 'text-slate-300';
});

const hamburgerBarClasses = computed(() => {
  return isNavy.value ? 'bg-white' : 'bg-slate-300';
});
</script>
  ...
  >
    ...
    >
      ...
      <div class="flex items-center">
        <RouterLink
          :to="homeRoute"
          class="group flex items-center gap-3.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-indigo focus-visible:ring-offset-2 rounded-btn"
      ...
      <div class="flex items-center gap-4">
        ...
        <div v-if="$slots.actions" class="hidden md:flex items-center gap-3">
          <slot name="actions" />
        </div>
        ...
        <button
          type="button"
          class="inline-flex h-10 w-10 items-center justify-center p-1 transition-opacity focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-indigo hover:opacity-80"
        ...
        >
          <div class="flex w-6 flex-col justify-center gap-[5px]" aria-hidden="true">
            <span
              class="block h-[3px] w-full shrink-0 rounded-none transition-transform duration-200 origin-center"
              :class="[
                hamburgerBarClasses,
    ...
    >
      <div
        ...
        :class="isNavy ? 'bg-brand-navy text-white' : 'bg-white text-slate-900 border-slate-200'"
        ...
        <div class="mx-auto max-w-7xl space-y-4">
          <slot name="menu" :close="closeMenu">
            <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4">
              <RouterLink
                to="/"
                class="block rounded-btn px-4 py-2.5 text-sm font-medium transition-colors hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-indigo"
                @click="closeMenu"
              >
                Beranda
              </RouterLink>
