...
describe('OtpVerificationModal network requests', () => {
  it('sends credentials on verify-otp and resend-otp so auth cookies are attached cross-origin', async () => {
    ...
    const verifyCall = fetchMock.mock.calls.find(([reqInput]) =>
      String(reqInput).includes('/api/auth/verify-otp'),
    );
  ...
  it('sends credentials on resend-otp', async () => {
    ...
    const resendCall = fetchMock.mock.calls.find(([reqInput]) =>
      String(reqInput).includes('/api/auth/resend-otp'),
    );
